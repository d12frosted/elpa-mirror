...
;; Package-Version: 20210924.6
;; Package-Commit: a513ec54e1fc642e724f9f0420826a6941c7a195
...
(defvar spell-fu--global-timer-refresh-buffers nil)

(defvar spell-fu--global-timer nil)

(defun spell-fu--refresh-buffer-add (buf)
  "Ensure BUF will be refreshed."
  (add-to-list 'spell-fu--global-timer-refresh-buffers buf nil #'eq))

(defun spell-fu--refresh-buffer-remove (buf)
  "Remove BUF from the list of buffers to refresh."
  (setq spell-fu--global-timer-refresh-buffers (delq buf spell-fu--global-timer-refresh-buffers)))
...
(defun spell-fu--time-callback-or-disable ()
  ...
  (let
    (
      ...
      (buf-curr-in-mode (bound-and-true-p spell-fu-mode)))
    ;; Ensure this buffer runs last in the current active window.
    (when buf-curr-in-mode
      (spell-fu--refresh-buffer-remove buf-curr))
    (when spell-fu--global-timer-refresh-buffers
        ...
        (dolist (win (window-list frame -1))
          (let ((buf (window-buffer win)))
            (when (buffer-local-value 'spell-fu-mode buf)
              (when (memq buf spell-fu--global-timer-refresh-buffers)
                (with-selected-frame
                  frame
                  (with-selected-window
                    win
                    (with-current-buffer buf (spell-fu--idle-handle-pending-ranges)))))))))

      ;; Always keep the current buffer in the list
      ;; so navigating away from this buffer will refresh it.
      (setq spell-fu--global-timer-refresh-buffers
        (if buf-curr-in-mode
          (list buf-curr)
          nil)))
    (cond
...
(defun spell-fu--time-reset ()
  ...
  (cond
    ((bound-and-true-p spell-fu-mode)
      (spell-fu--refresh-buffer-add (current-buffer))
      (spell-fu--time-ensure t))
    (t
      (spell-fu--time-ensure nil))))

(defun spell-fu--time-ensure-buffer-local-state (state)
  ...
  (cond
    ...
    (t
      (spell-fu--refresh-buffer-remove (current-buffer))
      (spell-fu--time-ensure nil)
...
(defun spell-fu--idle-enable ()
  ...
  (spell-fu--time-ensure-buffer-local-state t))
...
(defun spell-fu--idle-disable ()
  ...
  (spell-fu--time-ensure-buffer-local-state nil))
