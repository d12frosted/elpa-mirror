...
q[con] process/buffer.  `C-c C-j' (`C-c C-l' `C-M-x') sends a
single line, `C-c C-f' sends the surrounding function, `C-c C-r'
sends the selected region and `C-c C-b' sends the whole buffer.  If
prefixed with `C-u C-u', or pressing `C-c M-j' `C-c M-f' `C-c M-r'
