...
;; Package-Version: 20221230.1050
;; Package-Commit: 9c1b25a33a40cca016af35ca80baa4504b3f136c
;; Version: 0.2
      ...
      (save-excursion
        (save-match-data
          ...
          (let ((word-init (dabbrev--abbrev-at-point)))
            ...
            ;; Exclude this word from the list of options (if it exists at all).
            (setq result-choices
                  (recomplete--rotate-list-by-elt-and-remove
                   result-choices
                   word-init)))
                               ...
                               (lambda (iter-word)
                                 (prog1 (cond
                                         ((eq iter-index cycle-index)
                                          (format "[%s]"
                                                  (propertize iter-word 'face 'match)))
                                         (t
                                          (format " %s " iter-word)))
