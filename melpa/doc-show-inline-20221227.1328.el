...
;; Package-Version: 20221227.1328
;; Package-Commit: 17d24345b8cef86c25e69cb1642d58457f87718a
  ...
  (when (eq 'unspecified (face-attribute 'doc-show-inline-face :background))
    (let*
        ( ;; Tint the color.
         ...
         (default-tint
          (doc-show-inline--color-highlight default-color doc-show-inline-face-background-highlight)))
              ...
              (with-current-buffer buf
                ...
                (pcase-let ((`(,pos-beg . ,pos-end) (funcall doc-show-inline-extract-doc sym)))
                  (when (and pos-beg pos-end)
                    ...
                    (let ((text (doc-show-inline--buffer-substring-with-overlay-props pos-beg pos-end)))
    ...
    (let ((overlays-beg (point-max))
      ...
      (save-excursion
        ...
        (let ((points (funcall doc-show-inline-locations (point-min) (point-max))))
          (doc-show-inline--log-info "found %d identifier(s) in %S" (length points) (current-buffer))
          (when points
            (let
...
(defun doc-show-inline--idle-font-lock-region-pending (pos-beg pos-end)
  ...
  (when doc-show-inline--idle-overlays-debug
    (setq doc-show-inline--idle-overlays-debug-index (1+ doc-show-inline--idle-overlays-debug-index))
    ...
    (let ((ov (make-overlay pos-beg pos-end)))
      ...
      (overlay-put
       ...
       (list
        :background
        (nth doc-show-inline--idle-overlays-debug-index doc-show-inline--idle-overlays-debug-colors)
        :extend t)))))
