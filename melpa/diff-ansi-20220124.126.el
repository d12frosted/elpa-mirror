...
;; Package-Version: 20220124.126
;; Package-Commit: 1bf285fd9d1a66bf5bed715d54d5745027c341aa
;; Version: 0.1
...
(defcustom diff-ansi-chunks-size 8192
        ...
        (when face
          (unless (diff-ansi--face-has-bg face)
            (setq face
              (cond
                ((listp face)
                  (append face diff-ansi--ansi-color-bg))
                (t
      ...
      (while args
        (let ((arg-current (car args)))
          ...
          (cond
            ...
            (t
              (error
                "Arguments must be property pairs or lists of strings, found %S = %S"
                (type-of arg-current)
                arg-current)))))
    ...
    (while args
      (let ((arg-current (car args)))
        ...
        (cond
          ...
          (t
            (error
              "All arguments must be property pairs, found %S = %S"
              (type-of arg-current)
              arg-current)))))
