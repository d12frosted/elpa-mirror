...
;; Package-Version: 20221227.1156
;; Package-Commit: 6e12757facc0647e5a6ade9c4696da16ffdbb28c
;; Version: 0.1
...
(defun xref-rst--project-vars (error-prefix)
  ...
  (let*
      ((buf
        (or (current-buffer) (user-error (concat error-prefix "current buffer not found, exiting!"))))
       (current-filepath
        (or
          ...
          (cond
           ...
           ((string-match xref-rst--regex-any-role-data symbol)
            (let ((rst-role-id (match-string-no-properties 1 symbol))
                  (rst-role-data
                   (or (match-string-no-properties 2 symbol) (match-string-no-properties 3 symbol))))
              ...
              (cond
               ...
               ((string-equal rst-role-id "doc")
                (setq matches (xref-rst--lookup-doc current-project-root current-dir rst-role-data))))))
           ...
           (t
            (let ((rst-found-file
              ...
              (unless (file-exists-p rst-found-file)
                (user-error (concat error-prefix "not over an RST role or filename at the cursor")))
...
(defun xref-rst--find-references-to-term (symbol)
  ...
  (let ((error-prefix "RST-term-usage: ")
    ...
    (pcase-let ((`(,current-project-root ,_current-dir ,_current-filepath)
          ...
          (pcase-let ((`(,symbol-buffer . ,symbol-point) symbol-context))
            (with-current-buffer symbol-buffer
              (when (xref-rst--is-point-in-glossary-body symbol-point)
                (pcase-let ((`(,beg . ,end) (xref-rst--range-of-block-at-current-indent symbol-point)))
                  (setq rst-terms-data
                        (mapcar
                         (lambda (str)
                           (string-trim
      ...
      (when rst-terms-data
        (let
            ((rst-terms-data-regex (concat "\\(" (mapconcat #'regexp-quote rst-terms-data "\\|") "\\)"))
             (all-files
              (let ((case-fold-search t)) ;; Case insensitive search.
                (xref-rst--dirs-recursive current-project-root))))
    ...
    (cond
     ...
     (t
      (user-error
       (concat
        "RST usage: "
        (format "unable to determine a reference type from %S" (substring-no-properties symbol))))))
