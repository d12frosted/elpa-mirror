...
;; Package-Commit: 669b381e621749ccfe2ddf8af9d31c5721b965f3
;; Version: 0.1
      ...
      (save-excursion
        (while
          ...
          (let* ((diff-hunk-point (match-beginning 1))
                 (diff-hunk-begin
                  (string-to-number (buffer-substring-no-properties (match-beginning 4) (match-end 4))))
                 (diff-hunk-lines
                  (string-to-number (buffer-substring-no-properties (match-beginning 5) (match-end 5))))
        ...
        (cond
         ...
         (t
          (when current-filename
            (let* ((current-filename-relative (file-relative-name current-filename default-directory))
                   (point-found
