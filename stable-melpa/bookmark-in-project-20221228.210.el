...
;; Package-Version: 20221228.210
;; Package-Commit: 9c2432ae449ad6d30149100de327b497d3cf0c87
        ...
        (let ((lines-rel (count-lines pos-best pos)))
          ...
          (when (< 1 lines-rel)
            (let ((lines-all (count-lines pos-best pos-next)))
              (setq text (concat text " [" (bookmark-in-project--repr-precent lines-rel lines-all) "]")))))
...
(defun bookmark-in-project--name-impl (bm-list)
  ...
  (bookmark-in-project--name-ensure-unique
   (or
    (funcall bookmark-in-project-name)
    (bookmark-in-project-name-default-with-line))
   bm-list))
...
Returning the next bookmark or nil."
  (let* ((pi-best nil)
         (filepath-pi-list-pairs
          (bookmark-in-project--pretty-items-by-file-vector pi-list
                                                            (list
                                                             filepath-current)))
              ...
              (bookmark-in-project--message "(%s) %d of %d: %s%s"
                                            (cond
                                             ...
                                             (t
                                              "next"))
                                            (1+
                                             (bookmark-in-project--calc-global-index pi-list pi))
                                            (length bm-list)
  ...
  (let ((proj-dir (bookmark-in-project--project-root-impl)))
    (let ((bm-list
           (bookmark-in-project--filter-by-narrowing
            (bookmark-in-project--filter-by-project
             proj-dir
             bookmark-alist))))
      (cond
       ((null bm-list)
    ...
    (let ((bm-list
      ...
      (let
          ( ;; Already sorted.
           ...
           (bookmark-current-bookmark (bookmark-in-project--name-abbrev-or-nil proj-dir (bookmark-in-project--default-name-at-point))))
        ...
        (when bookmark-current-bookmark
          (setq bm-current-bookmark-new (bookmark-in-project--remap-name bookmark-alist bm-list bookmark-current-bookmark))))
...
only bookmarks on the current line will be considered."
  (let ((current-filename
         (bookmark-in-project--item-get-filename
          (bookmark-in-project--placeholder-item
           1))))
    (cond
