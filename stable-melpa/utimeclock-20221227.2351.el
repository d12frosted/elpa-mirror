...
;; Package-Version: 20221227.2351
;; Package-Commit: f7ff6274727e51e7441f8c08ef04ef31e1b70cea
;; Version: 0.1
  ...
  (let ((time-pair-sep (regexp-quote utimeclock-time-pair))
    ...
    (dolist (time-pair (split-string line))
      ...
      (pcase-let ((`(,time-start ,time-end) (split-string time-pair time-pair-sep)))
        ...
        (let ((time-span
               (-
                (utimeclock-as-sec-total time-end)
                (utimeclock-as-sec-total time-start))))
          ;; Wrap
          (when (< time-span 0)
            (setq time-span
                  (+
    ...
    (let ((line (string-trim-right (buffer-substring-no-properties pos (line-end-position)))))
      ...
      (when (string-suffix-p utimeclock-line-separator line)
        ...
        (when (zerop (forward-line 1))
          (setq line (concat line " " (utimeclock-extract-line-multi (line-beginning-position) prefix)))))
      line)))
