...
;; Package-Version: 20221208.1154
;; Package-Commit: 7bf9ed72a7b08b20107b86abd614b0fe9cbf01ed
...
(defcustom ox-spectacle-export-level 1
  "Export strategy.
1 for normal, 2 for embed scripts, 3 for embed scripts and images."
  :type 'integer)
...
(defun ox-spectacle--extract-options (opts info &optional normed)
  "Return the The first part of OPTS maybe the tag, others are props.
Separate and return them as cons. Assume the component name is capitalized
and html tags are all lower case. If NORMED non-nil then filter the props.
INFO is a plist holding export options."
...
contextual information."
  (if (and (>= (ox-spectacle--export-level info) 3) (> (length props) 10))
  ...
  (let ((title (org-export-data (org-element-property :title headline) info))
    ...
    (if (string-equal-ignore-case (org-element-property :raw-value (car headlines)) "<config>")
      ...
      (let* ((props (ox-spectacle--wa (org-element-property :PROPS headline)))
             ...
             prefix inline-tag inline-props)
...
contextual information."
  (let* ((lang (org-element-property :language element))
    ...
    ;; catch config scripts and others under <config> section
    (if (string-equal-ignore-case (org-element-property :raw-value root) "<config>")
      ...
      (let ((contents
             (format "<${Box}%s>\n<${CodePane}%s%s%s>\n${`\n%s\n`}\n</${CodePane}>\n</${Box}>"
                     ...
                     code-props code)))
        (ox-spectacle--maybe-appear contents appear-flags)))))
...
the plist used as a communication channel."
  (let* ((headline (org-element-lineage paragraph '(headline)))
    ...
    (if (or (eq parentype 'item)
      ...
      (if (string-match-p regexp (or tag "")) (setq tag (format "${%s}" tag)))
