...
;; Package-Version: 20210922.110
;; Package-Commit: 74360c1b4aa384783dca8011211f182e190bfca2
...
(defun spell-fu--idle-handle-pending-ranges-or-disable ()
...
This can happen when switching buffers."
  (cond
    ...
    ((and (bound-and-true-p spell-fu-mode))
      (spell-fu--idle-handle-pending-ranges))
    ;; The global idle timer has run in a new buffer,
    ;; disable the idle timer.
    (t
      (spell-fu--idle-timer-ensure nil))))

(defun spell-fu--idle-timer-ensure (state)
  "Add the global idle timer is enabled if STATE is non-nil."
  (cond
    (state
      (unless spell-fu--idle-timer
        (setq spell-fu--idle-timer
          (run-with-idle-timer
            spell-fu-idle-delay
            t
            #'spell-fu--idle-handle-pending-ranges-or-disable))))
    (t
      (when spell-fu--idle-timer
        (cancel-timer spell-fu--idle-timer)
        (setq spell-fu--idle-timer nil)))))

(defun spell-fu--idle-timer-reset-on-state-change ()
  "Run this when the buffer changes."
  (spell-fu--idle-timer-ensure (bound-and-true-p spell-fu-mode)))
...
(defun spell-fu--idle-enable ()
  ...
  (spell-fu--idle-timer-ensure t)
  (add-hook 'window-state-change-hook #'spell-fu--idle-timer-reset-on-state-change nil t))
...
(defun spell-fu--idle-disable ()
  ...
  (remove-hook 'window-state-change-hook #'spell-fu--idle-timer-reset-on-state-change t))
