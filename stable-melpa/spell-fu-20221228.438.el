...
;; Package-Commit: 24e389a24cd849410331919949d5dfe2811a55d9
  ...
  (cond
   (state
    (unless spell-fu--global-timer
      (setq spell-fu--global-timer (run-with-idle-timer spell-fu-idle-delay :repeat #'spell-fu--time-callback-or-disable))))
   (t
    ...
    (save-match-data
      (dolist (tmp-path (list ispell-aspell-dict-dir ispell-aspell-data-dir))
        ...
        (dolist (dict-re (list "^[[:alpha:]]+" "^[[:alpha:]_]+" "^[[:alpha:]]+-\\(alt\\|old\\)"))
          (let ((dict-match
                 (and (string-match dict-re dict-aspell-name)
                      (match-string 0 dict-aspell-name))))
            (when dict-match
