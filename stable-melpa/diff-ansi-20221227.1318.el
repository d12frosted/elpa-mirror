...
;; Package-Version: 20221227.1318
;; Package-Commit: ac8b7cf784bce7f869b208c11d79372805e99d3a
;; Version: 0.2
...
(defconst diff-ansi--code-block-for-multiprocess-defs
  ...
  (quote (progn
           ...
           (defun diff-ansi--ansi-color-apply-on-region-with-bg-impl (beg end)
             (let ((ansi-color-apply-face-function #'diff-ansi--ansi-color-apply-text-property-face--local))
            ...
            (lambda (proc _msg)
              (let ((id (process-get proc :my-job-id)))
                ...
                (when progress-message
                  (let ((message-log-max nil))
                    (message "%s%.2f%%" progress-message (* 100.0 (/ (float count-complete) args-len))))))))
...
(defun diff-ansi--command-preset-impl ()
  ...
  (pcase diff-ansi-tool
    ('delta
     (append
      (list "delta" (format "--width=%d" (window-body-width (get-buffer-window (current-buffer)))))
      ...
      (let ((inhibit-read-only t))
        (diff-ansi--with-temp-directory temp-dir
          ...
          (with-temp-buffer
            ...
            (goto-char (point-min))
            (let*
                ((black-color (diff-ansi--face-attr-to-hex-literal 'diff-ansi-default-face :background))
