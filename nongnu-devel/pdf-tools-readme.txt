...
7.5 On macOS (Homebrew), pkg-config cannot find poppler when building epdfinfo
──────────────────────────────────────────────────────────────────────────────

  If `make -s' or `M-x pdf-tools-install' fails with errors like
  `Package 'poppler' not found' or `Package 'gpgmepp', required by
  'poppler', not found', the most common cause is that *a non-Homebrew
  `pkg-config' is being used* (e.g., from Anaconda/Miniconda).
  ...
  *Step 1: Check which `pkg-config' is being used*
  ...
  If this shows something other than `/opt/homebrew/bin/pkg-config'
  (Intel Macs: `/usr/local/bin/pkg-config'), that's likely the problem.
  ...
  *Step 2b: Fix for `M-x pdf-tools-install' within Emacs*
  ...
  If Emacs is picking up the wrong `pkg-config', add this to your init
  file (thanks to @eli-tziperman):
  ┌────
  ...
  *Note*: The `./server/autobuild' script already adjusts
   `PKG_CONFIG_PATH' on macOS for keg-only packages, so prefer running
   it directly if you encounter path-related issues.


7.6 I am on a Macbook M1 and pdf-tools installation fails with a stack-trace
────────────────────────────────────────────────────────────────────────────
...
7.7 I am a developer, making changes to the pdf-tools source code
─────────────────────────────────────────────────────────────────
...
7.9 How do I open the current PDF in an external application?
─────────────────────────────────────────────────────────────
