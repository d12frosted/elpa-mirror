...
─────
  ...
  While the mode is active, the `C-c C--' key inserts the package
...
────────────────────────────
  ...
  `nameless-mode' binds the `C-c C--' key to `nameless-insert-name',
  which immediately inserts the current name for you, or even expands
  aliases to the names they point to.
  ...
  You can bind this to the `_' key and make it even faster to insert the
  name.
...
──────────────────────────────
  ...
  In these situations, simply set `nameless-current-name' as file-local
  variable.  To do that, invoke the following command:
  ┌────
...
───────────────────────────────────
  ...
  You can also configure it for a whole project, by setting it as a
  dir-local variable.
